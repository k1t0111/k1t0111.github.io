...
```shell
docker build -t ctf:v1 .            # dockerfile目录下输入构建命令，用来构建基于ubuntu:16.04的自定义镜像。 docker run -d -p 5555:80 -e FLAG=flag{$(cat /proc/sys/kernel/random/uuid)}
镜像id  # 添加系统docker run -d --name <容器名称> <镜像名称>:<标签>uuid作为动态flag字段docker ps                           # 查看是否运行docker exec -it 容器id /bin/bash     # 进入容器exit                                 # 在容器内退出
```
...
```shell
docker stop 容器id                     #停止指定容器docker stop $(docker ps -a -q)      #停止所有已经运行的容器startdocker rm 容器id                       #删除指定容器，删除前先停止docker rm $(docker ps -a -q)        #删除所有已经停止的指令docker rmi 镜像id                    # 删除指定镜像docker rmi -f $(docker images-q)
删除全部 -f 强制
```

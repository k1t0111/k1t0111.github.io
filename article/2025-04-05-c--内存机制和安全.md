...
```
```c
#include <iostream>#include <string>// 定义模板函数template <typename T>void print(const T& value)
{
    std::cout << "Value: " << value << std::endl;}int main()
    {
        int i = 42;
        double d = 3.14;
        std::string s = "Hello, Template!";
...
if (ptr)        {
    ...
    delete[] ptr;
    ptr = nullptr;
    // 避免悬空指针        }/*	这里面有个很重要的观点，delete 只是清理了内存但是指针指向的内存并没有清理。	于是我们还需要将指针置空  */
```
...
noexcept : m_data(other.m_data), m_size(other.m_size)    {
    other.m_data = nullptr;
    // 避免悬空指针        other.m_size = 0;
    cout << "移动构造函数被调用" << endl;
}
```
...
// buf1 是左值SecureBuffer buf2 = std::move(buf1);
...
buffer; // 忘记释放原有资源        buffer = new char[other.size];        size = other.size;        memcpy(buffer, other.buffer, size);    }    return *this;}
```
...
// 动态数组std::vector<int> vec;vec.push_back(1);
// 动态添加元素int x = vec[0];
// 访问第一个元素
```
